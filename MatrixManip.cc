...
void MatrixManip::discard_vars_with_too_few_obs(const boost::uint64_t min_non_zero, Matrix& mat)
{
    boost::uint64_t num_vars_kept = 0;

    for (boost::uint64_t curr_var = 0; curr_var < mat.num_vars(); curr_var++)
    {
        boost::uint64_t num_non_zero = 0;
        boost::uint64_t curr_obs = 0;
        ...
        if (num_non_zero >= min_non_zero)
        {
            ...
            mat.var_name(num_vars_kept) = mat.var_name(curr_var);
...
void MatrixManip::compute_ranks(Matrix& mat)
{
    #pragma omp parallel
    {
        std::vector<boost::uint64_t> var_ranks(mat.num_obs());
        ...
        for (boost::uint64_t curr_var = 0; curr_var < mat.num_vars(); curr_var++)
        {
            ...
            for(boost::uint64_t i = 0; i < mat.num_obs(); i++)
            {

                const boost::uint64_t start = i;
                while(i < (mat.num_obs()-1) &&
                        (mat.data(curr_var, var_ranks[i]) == mat.data(curr_var, var_ranks[i+1])))
                {
                    i++;
                }
                const boost::uint64_t end = i;


                double final_rank = (start+end)/2.0;

                for (boost::uint64_t j = start; j <= end;j++)

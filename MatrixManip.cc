...
void MatrixManip::discard_vars_with_too_few_obs(const size_t min_non_zero, Matrix& mat)
{
    size_t num_vars_kept = 0;
...
void MatrixManip::compute_ranks(Matrix& mat)
{
    #pragma omp parallel
    {
        std::vector<size_t> var_ranks(mat.num_obs());
        ...
        for (size_t curr_var = 0; curr_var < mat.num_vars(); curr_var++)
        {
            ...
            for(size_t i = 0; i < mat.num_obs(); i++)
            {
